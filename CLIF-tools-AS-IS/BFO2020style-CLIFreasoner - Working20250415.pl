...
:- dynamic r/6,				% rule from Kowalski file
		   ...
		   eqp/3,			% equivalent predicates
							%	arg1=axiomID in which defined
							%	arg2=equivalent predicate to remove
							%	arg3=equivalent predicate to keep
...
:-writeln("BFO2020 Style CLIF reasoner, Version January 24, 2024").
...
run:-
	 ...
	 start_conditions_ok(Out),
% from here to next '%end' should move to the parser
	 writeln("Optimizing Kowalski input ..."),
	 write_dupl(Out),
	 detect_equivalent_predicates,	%trace,
	 assert(pl(0)),				% initialize arity value
	 list_question_types, 		% process kowalski-input  move this to parser
...
% i
write_dupl(Name):-
   string_concat(Name,"-double_clauses.txt",File),
   write("Saving replicated clauses to "), writeln(File),
...
% i
duplicate_rules(Out):-
	r(_, _, _, _, _, kow(IF, Con)),
	setof([N, Axiom], r(N, _, _, _, Axiom, kow(IF, Con)), Xs),
	length(Xs, L),
	L>1,
	writeln(Out,""),
	ana_dup(Out,Xs,1),
...
detect_equivalent_predicates:-
	r(_,1,[],[],AxID,kow(if([A]),then(C))),		% pick the first rule with one antecedent and one conclusion
	\+eqp(AxID,_,_),							% if the axiom to which the rule belongs has not yet been processed
	r(_,1,[],[],AxID,kow(if([C]),then(A))),		% pick a 2nd rule from that axiom representing a biconditional with the 1st one
	term_variables(A,AV), sort(AV,AVs),			% isolate and sort the variables from the 1st rule's atecedent 
	term_variables(C,CV), sort(CV,CVs),			% idem for the consequent
	A=[AF|_], C=[CF|_],							% extract the relation (predicate) names
	AF\==CF,									% if they are different
	(	AVs =@= CVs								%	and if the same variables figure in antecedent and consequent
			->	assertz(eqp(AxID,A,C))			% store axiomID, antededent and consequent for later use
	),
